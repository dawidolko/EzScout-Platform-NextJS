...
import { FormularzKontaktu } from './FormularzKontaktu';

/*
 * Jedna strona obslugujaca oba jezyki. Trasy `/` i `/pl` roznia sie wylacznie
 * obiektem `t`, wiec zmiana ukladu dotyczy od razu obu wersji i nie da sie
 * zapomniec o jednej z nich.
 */
    ...
    <>
        ...
        <div className="relative mx-auto grid max-w-6xl gap-12 px-4 py-20 sm:px-6 lg:grid-cols-[1.15fr_1fr] lg:py-28">
          <div>
            ...
            <h1 className="mt-4 font-display text-5xl font-bold leading-[1.05] text-ink-50 sm:text-6xl lg:text-7xl">
              {t.hero.tytul1}
              <br />
              <span className="text-amber-400">{t.hero.tytul2}</span>
            </h1>
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-ink-300">{t.hero.opis}</p>

            <div className="mt-9 flex flex-wrap gap-3">
              <a href="#oferta" className="rounded-md bg-amber-500 px-6 py-3 font-bold text-ink-950 transition hover:bg-amber-400">
                {t.hero.ctaGlowne}
              </a>
              <a href="#jak-dzialamy" className="rounded-md border border-ink-600 px-6 py-3 font-bold text-ink-100 transition hover:border-amber-400 hover:text-amber-400">
                {t.hero.ctaDrugie}
              </a>
            </div>

            <dl className="mt-12 grid grid-cols-3 gap-6 border-t border-ink-700 pt-8">
              {t.liczby.slice(0, 3).map((l) => (
                <div key={l.etykieta}>
                  <dt className="text-xs uppercase tracking-wider text-ink-400">{l.etykieta}</dt>
                  <dd className="tabular mt-1 font-display text-3xl font-bold text-ink-50">{l.wartosc}</dd>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div className="lg:pl-4">
            <div className="rounded-xl border border-ink-700 bg-ink-800 p-6 shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b border-ink-700 pb-4">
                <div>
                  <p className="text-xs uppercase tracking-wider text-ink-400">{t.hero.kartaNadtytul}</p>
                  <p className="font-display text-2xl font-bold text-ink-50">{t.hero.kartaTytul}</p>
                </div>
                <span className="rounded bg-grass-500/15 px-2.5 py-1 text-xs font-bold text-grass-500">
                  {t.hero.kartaZnacznik}
                </span>
              </div>

              <dl className="mt-5 space-y-4">
                {t.hero.kartaOceny.map(([nazwa, wynik]) => (
                  <div key={nazwa}>
                    ...
                    <div
                      ...
                      role="img"
                      aria-label={`${nazwa}: ${wynik}/100`}
                    >
                      <div className="h-full rounded-full bg-amber-500" style={{ width: `${wynik}%` }} />
                    </div>
                  </div>
                ))}
              </dl>
      ...
      <section id="oferta" aria-labelledby="oferta-h" className="border-b border-ink-800 py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <p className="mt-3 max-w-2xl text-ink-300">{t.oferta.opis}</p>
            ...
            {t.pakiety.map((p) => (
              <li
                ...
                className={`flex flex-col rounded-xl border p-6 ${
                  p.wyrozniony ? 'border-amber-500 bg-ink-800 ring-1 ring-amber-500/30' : 'border-ink-700 bg-ink-900'
                }`}
              >
                {p.wyrozniony && (
                ...
                <p className="tabular mt-2 font-display text-3xl font-bold text-amber-400">{p.cena}</p>
                ...
                >
                  {t.oferta.zamow}{' '}
                  <span className="sr-only">{t.oferta.zamowSr.replace('{n}', p.nazwa)}</span>
                </a>
              </li>
            ))}
          </ul>
      ...
      {/* ---------------- Jak dzialamy ---------------- */}
      <section id="jak-dzialamy" aria-labelledby="jak-h" className="border-b border-ink-800 bg-ink-900 py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <h2 id="jak-h" className="font-display text-4xl font-bold text-ink-50 sm:text-5xl">
            {t.jak.tytul}
          </h2>
          <p className="mt-3 max-w-2xl text-ink-300">{t.jak.opis}</p>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="grid gap-12 lg:grid-cols-[1fr_1.1fr]">
            <div>
              ...
              <p className="mt-4 leading-relaxed text-ink-300">{t.skutecznosc.akapit1}</p>
              <p className="mt-4 leading-relaxed text-ink-300">{t.skutecznosc.akapit2}</p>
            </div>
            ...
            <dl className="grid grid-cols-2 gap-5">
              {t.liczby.map((l) => (
                <div key={l.etykieta} className="rounded-xl border border-ink-700 bg-ink-900 p-6">
                  <dt className="text-xs uppercase tracking-wider text-ink-400">{l.etykieta}</dt>
                  <dd className="tabular mt-2 font-display text-4xl font-bold text-amber-400">{l.wartosc}</dd>
            ...
            {t.faq.pytania.map((p) => (
              <details key={p.pytanie} className="group py-4">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-semibold text-ink-50">
                  {p.pytanie}
                  <span aria-hidden="true" className="shrink-0 text-2xl leading-none text-amber-400 transition group-open:rotate-45">
                    +
                  </span>
                </summary>
      ...
      <section id="kontakt" aria-labelledby="kontakt-h" className="py-20">
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <p className="mt-3 text-ink-300">{t.kontakt.opis}</p>
          <FormularzKontaktu t={t} />
        </div>
      </section>
    </>
  );
